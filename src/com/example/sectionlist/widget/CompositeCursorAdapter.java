...
public abstract class CompositeCursorAdapter extends BaseAdapter {
	...
	public void removePartition(int partitionIndex) {
		...
		System.arraycopy(mPartitions, partitionIndex + 1, mPartitions,
				partitionIndex, mSize - partitionIndex - 1);
		mSize--;
		invalidate();
		notifyDataSetChanged();
	}
	...
	public void changeCursor(int partition, Cursor cursor) {
		...
		if (prevCursor != cursor) {
			...
			if (cursor != null) {
				mPartitions[partition].idColumnIndex = cursor
						.getColumnIndex("_id");
			}
			invalidate();
			notifyDataSetChanged();
...
//		Log.w("getView", "getView: position: " + position + ", convertView: "
//				+ convertView);
		int start = 0;
		for (int i = 0; i < mSize; i++) {
			int end = start + mPartitions[i].count;
			if (position >= start && position < end) {
				...
				View view;
				if (offset == -1) {
					view = getHeaderView(i, mPartitions[i].cursor, convertView,
							parent);
				}
				else {
					if (!mPartitions[i].cursor.moveToPosition(offset)) {
						throw new IllegalStateException(
								"Couldn't move cursor to position " + offset);
					}
					view = getView(i, mPartitions[i].cursor, offset,
							convertView, parent);
				}
				if (view == null) {
	 ...
	 * needed.
	 */
	protected View getHeaderView(int partition, Cursor cursor,
			View convertView, ViewGroup parent) {
//		Log.i("getView", "getHeaderView: partition: " + partition
//				+ ", convertView: " + convertView);
		View view = convertView != null ? convertView : newHeaderView(mContext,
				partition, cursor, parent);
	 ...
	 */
	protected abstract View newHeaderView(Context context, int partition,
			Cursor cursor, ViewGroup parent);

	/**
	 * Binds the header view for the specified partition.
	 */
	protected abstract void bindHeaderView(View view, int partition,
			Cursor cursor);

	/**
	 * Returns an item view for the specified partition, creating one if needed.
	 */
	protected View getView(int partition, Cursor cursor, int position,
			View convertView, ViewGroup parent) {
		// Log.i("getView", "getContentView: partition: " + partition
		// + ", position: " + position + ", convertView: " + convertView);
		View view;
		if (convertView != null) {
			view = convertView;
		}
	 ...
	 */
	protected abstract View newView(Context context, int partition,
	 ...
	 * corresponds directly to the current cursor position.
	 */
	protected abstract void bindView(View v, int partition, Cursor cursor,
			int position);
